...
{
    __step_counter = 0;
    __last_outlier_ammount = 0;
    __min_steps_for_deviation_update = 40.0f;
    __outlier_Threshold = 2.f;
...
void MovementDetector::getOutlierMask( cv::Mat& outlierMask )
{
    double minV, maxV;
...
void MovementDetector::getMeanImage( cv::Mat& meanImage )
{
    double minV, maxV;
...
bool MovementDetector::addImage(const cv::Mat& greyImage)
{
    __last_outlier_ammount = 0;
    ...
    else
    {
        ...
        if(__last_outlier_ammount * 100 / (__normalGreyImage.rows*__normalGreyImage.cols) > __outliers_percent_threshold)
        {
...
//    // (DEBUG) Show image values
//    imshow("Mean image", __mean);
//    imshow("Outlier mask", __outlier);
    
    return oclussion_occurred;
}

void MovementDetector::normalizeImage(const cv::Mat& src_img, cv::Mat& dst_img)
{
...
bool MovementDetector::wasMovement( const float changedPixelPercentage )
{
    if(__last_outlier_ammount * 100 / (__normalGreyImage.rows*__normalGreyImage.cols) > changedPixelPercentage)
    {
        return true;
    }
